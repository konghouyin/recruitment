...
var zlib = require('zlib'); //gzip解密

module.exports = {
	prove: function(yhm, password, req, res, end) {

		function makeoptions(url, cookie, style) {
			return {
				method: style,
				url: url,
				headers: {
					'Postman-Token': 'b18095f8-79ce-41fc-8e46-3cba97caf402',
					'cache-control': 'no-cache',
					'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
					'Accept-Encoding': 'gzip, deflate',
					Referer: 'http://www.xiyou.edu.cn/xxfw/cyfw1.htm',
				...
				//proxy: "http://127.0.0.1:8888", //设置fiddler抓包
				encoding: null //保持body为字符流
			}
		};

		var getcookie = makeoptions('http://www.zfjw.xupt.edu.cn/jwglxt/', "", "GET");
		request(getcookie, function(error, response, body) {
			//请求-----session-id
			...
			var url = "http://www.zfjw.xupt.edu.cn/jwglxt/xtgl/dl_loginForward.html?language=&_t=" + new Date().getTime();
			var getTokenOptions = makeoptions(url, cookie, 'GET');
			request(getTokenOptions, function(error, response, body) {
				//请求-----csrftoken
				var csrftoken = getcsrftoken(body); //保存csrftoken
				...
				var url = "http://www.zfjw.xupt.edu.cn/jwglxt/xtgl/login_getPublicKey.html?time=" + new Date().getTime();
				var getPublicKey = makeoptions(url, cookie, 'GET');
				request.get(getPublicKey, function(error, response, body) {
					//请求-----加密公钥
					...
					function callback(text) {
						...
						var mm = makemm(obj.modulus, obj.exponent, password);
						...
						var options = {
							...
							headers: {
								...
								'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
								'Accept-Encoding': 'gzip, deflate',
								Referer: 'http://www.zfjw.xupt.edu.cn/jwglxt/xtgl/login_slogin.html?language=zh_CN&_t=1548933179789',
								Accept: 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
								'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36',
		...
		BigInteger.prototype.fromString =
			bnpFromString;
		...
		BigInteger
			.prototype.subTo = bnpSubTo;
		BigInteger.prototype.multiplyTo = bnpMultiplyTo;
		BigInteger.prototype.squareTo =
			bnpSquareTo;
		...
		BigInteger
			.prototype.compareTo = bnCompareTo;
		BigInteger.prototype.bitLength = bnBitLength;
		BigInteger.prototype.mod = bnMod;
		BigInteger
			.prototype.modPowInt = bnModPowInt;
