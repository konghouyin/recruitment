...
{
    msg："具体情况"，
    style：0	//1成功，0查无信息，-200session失效
    selfInfo：{查询结果的对象}
}
```
...
```json
{
    style: 1,//1成功，0查无信息，-200session失效
    msg: "查找成功",
    state: {状态对象}
}
```
...
- url：/passchange
- 方法：post
- 参数：old=xxx（MD5）&new=xxx（MD5）
- 返回

```json
{
    style: 1,//1成功，0失败，-1session记录电话不匹配，-2旧密码错误，-200session失效
    msg: "密码修改成功",
}
```
...
{
    msg："具体情况"，
    style：0	//1成功，0没有查到此学号，-1姓名与学号不匹配，-2账号已经绑定学号，-3数据库异常，-5此学号已绑定，-200session失效
}
```
...
{
    msg："具体情况"，
    style：0	//1成功，0用户名或密码有误，-2已经绑定学号，-3数据库异常，-4教务代理请求异常，-5此学号已绑定，-200session失效
}
```
...
{
    msg："具体情况"，
    style：0	//1成功，0报名已截止，-1已经分组，-2还未完成身份信息填写，-3数据库异常，，-200session失效，-4产品组大一报名
